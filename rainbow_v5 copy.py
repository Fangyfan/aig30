...
class RainbowDQN(nn.Module):
    def __init__(self, args, action_space):
        ...
        self.linear = nn.Linear(self.feature_space, self.feature_out)
        # 定义网络
        self.fc_h_v = NoisyLinear(
            self.feature_out, args.hidden_size, std_init=args.noisy_std
        )
        self.fc_h_a = NoisyLinear(
            self.feature_out, args.hidden_size, std_init=args.noisy_std
        )
        self.fc_z_v = NoisyLinear(
            args.hidden_size, self._atoms, std_init=args.noisy_std
        )
        self.fc_z_a = NoisyLinear(
            args.hidden_size, self._action_num * self._atoms, std_init=args.noisy_std
        )
