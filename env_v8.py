...
class Env(object):
	def __init__(self, aigfile, temp=None, length_max=20, temp_prefix=None, device="cpu"):
		"""在初始化后必须调用一次 reset"""
		self.device = device
		self._temp = temp
	...
	def reset(self):
		self._resyn2()
		self._resyn2()
		self._curNumAnd, self._curLev = self.get_stats()
	...
	def get_action_seq(self, _join=","):
		"""获取动作序列, 若想获取动作序列的序号, 直接调用 self.seqIndex"""
		seq = []
	...
	def print(self, path, epoch):
		"""输出动作序列到指定文件, 获取动作序列可用 get_action_seq"""
	...
	def step(self, actionIdx: int):
		"""执行一步动作, 并返回 nextState, reward, self.done"""
		# 判断任务耗时是否应该提前终止
	...
	def random(self, lut_out: float = None):
		"""
        随机返回一个动作
        @param lut_out 当 lut_out 不为 None 时会有概率返回 lut_out 的动作
	...
	def state(self):
		"""返回当前的 state, 包含一个 feature 和 graph"""
	...
	def get_stats(self):
		"""获取当前的 numAnd 和 lev"""
		curStats = self._abc.aigStats()
		return curStats.numAnd, curStats.lev
