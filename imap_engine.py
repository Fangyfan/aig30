...
class EngineIMAP():
    ...
    def writeAig(self, outfile: str = None):
        """ Write the aig file
        """
        if outfile is None:
            imap.write_aiger(filename=self.output_file)
            return self.output_file
        else:
            imap.write_aiger(filename=outfile)
            return outfile
    ...
    def rewrite(self, priority_size=10, cut_size=4, level_preserve=False, zero_gain=False, verbose=False):
        ...
        Args:
            ...
            level_preserve (bool, optional): Preserve depth after optimization. Defaults to False.
        ...
        """
        imap.rewrite(priority_size=priority_size, cut_size=cut_size,
                     level_preserve=level_preserve, zero_gain=zero_gain, verbose=verbose)
    ...
    def refactor(self, max_input_size=10, max_cone_size=16, level_preserve=False, zero_gain=False, verbose=False):
        ...
        Args:
            ...
            verbose (bool, optional): Verbose report. Defaults to False.
        """
        imap.refactor(max_input_size=max_input_size, max_cone_size=max_cone_size,
                      level_preserve=level_preserve, zero_gain=zero_gain, verbose=verbose)
    ...
    def lut_opt(self, priority_size=10, cut_size=6, global_area_iterations=1, local_area_iterations=2, zero_gain=False, verbose=False):
        ...
        Args:
            ...
            verbose (bool, optional): Verbose report. Defaults to False.
        """
        imap.lut_opt(priority_size=priority_size, cut_size=cut_size, global_area_iterations=global_area_iterations,
                     local_area_iterations=local_area_iterations, zero_gain=zero_gain, verbose=verbose)
    ...
    def history(self, clear=False, size=False, add=False, replace=-1, backup=-1):
        ...
        Args:
            ...
            add (bool, optional): Add current AIG into the history AIG. Defaults to False.
            replace (int, optional): Replace the given indexed history AIG by current AIG. Defaults to -1.
            backup (int, optional): Replace the current AIG by the previous optimized AIG. Defaults to -1.
        """
        imap.history(clear=clear, size=size, add=add,
                     replace=replace, backup=backup)
    ...
    def map_fpga(self, priority_size=10, cut_size=6, global_area_iterations=1, local_area_iterations=1, type=0, verbose=False):
        ...
        Args:
            ...
            verbose (bool, optional): Verbose report. Defaults to False.
        """
        imap.map_fpga(priority_size=priority_size, cut_size=cut_size, global_area_iterations=global_area_iterations,
                      local_area_iterations=local_area_iterations, type=type, verbose=verbose)
    ...
    def print_stats(self, type=0, file=0):
        ...
        """
        imap.print_stats(type=type, file=file)
